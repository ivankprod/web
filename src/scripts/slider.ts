...
export class SliderClass {
	...
	constructor(sliderContainerID: string, sliderConfig: SliderConfig) {
		...
		this.maxSlideHeight = Math.max.apply(null, this.slidesList.map(elem => elem.clientHeight));
		this.slidesList.forEach(elem => { elem.style.height = this.maxSlideHeight + "px"; });
